node {
  def project = 'gebeya-devops-trainer'
  def appName = 'web-admin-service'
  def nameSpace='staging'
  def cluster='jenkins-cd'
  def region='us-east1-d'
  def feSvcName = "srvcz-${appName}"
 ...
 // sh("gcloud config set compute/zone $region  && gcloud container clusters get-credentials $cluster")
  ...
  case "canary":
      ...
      sh("sed -i.bak 's#gcr.io/${project}/${appName}:1.0.0#${imageTag}#' ./k8s/${nameSpace}/*.yaml")
